...
local AutoGiftToggle = AutomationTab:CreateToggle({
   ...
   Callback = function(Value)
		...
		-- FIXED: removed getconnections() which is not supported in Delta X
		-- Using a simpler approach with ProximityPrompt or direct network fire
...
CopyButton = LayoutTab:CreateButton({
   ...
   Callback = function()
		if SelectedFloor and SelectedFloor >= 1 and SelectedFloor <= 10 then
			...
			-- FIXED: setclipboard compatibility for Delta X
			local encoded = HttpService:JSONEncode(Items)
			pcall(function()
				setclipboard(encoded)
			end)
			if not pcall(setclipboard, encoded) then
				pcall(function()
					toclipboard(encoded)
				end)
			end
...
function SendWebhook(skipError, receivedStats)
	...
	-- FIXED: Delta X compatible HTTP request
	local reqFunc = (syn and syn.request) or (http and http.request) or http_request or request
	if reqFunc then
		pcall(reqFunc, {
			Url = WebhookURL,
